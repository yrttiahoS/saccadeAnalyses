...
%Function [gaze_violation] = AOIBorderViolationDuringNonValidSection(DATA, xcol, ycol, valcol, aoicoord, accepted_validities)
%
% Returns a truth value if the gaze moves over the aoi border during 
% invalid data.

%disp('Calculating if a gaze moved over aoi border during invalid data...');


 %violation(x, y, validity, accepted_validities, aoicoord)
% calculate, for one vector if it violates the coordinates during nonvalid
% section. Cycle throught all the rows and after the first valid datapoint
% found start looking if the aoi border is violated during nvs.
