...
%        select = select(floor( length(select)/2)+1:length(select));
        ...
        if ~isempty(waves1)
           ...
           % 'Stimulus',
            ...
            % 'aoi_violé';
            outCell{i+1, find(strcmp(csvheaders,'aoi_viol'))} =  aoi_violation(i);
            
            
            % 'aoi_coords';
            outCell{i+1, find(strcmp(csvheaders,'aoi_coords'))} =  num2str(thisCalibdotAOI{i});
 
            % 'mean X';
            outCell{i+1, find(strcmp(csvheaders,'mean X'))} =  meanXCoord(i);
            
            % 'mean Y';
            outCell{i+1, find(strcmp(csvheaders,'mean Y'))} =  meanYCoord(i);
            
            % 'Fix duration';
            outCell{i+1, find(strcmp(csvheaders,'Fix duration'))} =  fixDuration(i);
            
            
            % 'mean X';
            outCell{i+1, find(strcmp(csvheaders,'mean X fix'))} =  meanXCoordFIX(i);
            
            % 'mean Y';
            outCell{i+1, find(strcmp(csvheaders,'mean Y fix'))} =  meanYCoordFIX(i);
            
            % ID
            [token, file] = strtok(cell2mat(filename(i)), '_');
            outCell{i+1, find(strcmp(csvheaders,'ID'))} = token;
            
            % Type
            [token, file] = strtok(file, '_');
            outCell{i+1, find(strcmp(csvheaders,'type'))} = token;
            [token, file] = strtok(file, '_');
            
            %ext
            [token, file] = strtok(file, '_');
            outCell{i+1, find(strcmp(csvheaders,'ext'))} = token;
...
% outputVersion = strrep(outputVersion, '      ', '');
% save(['outCell_' outputVersion '.mat'], 'outCell')                                                                                                                                              

% Write output data
xlswrite([outputfilename '.xls'],outCell);  %xlswrite(['outCell_' outputVersion],outCell);